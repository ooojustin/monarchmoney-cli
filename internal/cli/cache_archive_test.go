...
func TestCacheSearchOnOutdatedCachePromptsResync(t *testing.T) {
	...
	if err := h.execute("--json", "cache", "search", "coffee"); err != nil {
		t.Fatalf("execute() error = %v", err)
	}
	if h.ExitCode == 0 {
		t.Fatalf("exitCode = 0, want failure on outdated cache; output=%q", h.Stdout.String())
	}
	if !strings.Contains(h.Stdout.String(), "run 'monarch cache sync' to rebuild it") {
		t.Fatalf("output = %q, want re-sync guidance", h.Stdout.String())
	}
}
