...
- On `overview` and cashflow commands other than `trends`, an omitted `--from` means the first of the current month and an omitted `--to` means today, resolved against your local calendar. A lone `--from` therefore reads as "through today"; a lone `--to` resolves to an inverted range and exits 2. `cashflow trends` requires both bounds.
- `--limit` must be at least 1 and `--offset` must not be negative.
- A command that owns subcommands requires one. Invoked bare or with an unknown subcommand it exits 2 with `INVALID_ARGUMENTS`; `<parent> help` is an explicit help request and exits 0.
