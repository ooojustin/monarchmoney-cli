...
profile: personal
timeout: 60s
cache_path: ~/finances/cache/monarch.sqlite
backup_path: ~/finances/monarch.journal
```
...
- `backup_path` makes **every successful `monarch cache sync` regenerate your hledger journal automatically** (see [Ledger backup](ledger-backup.md)).

Two more keys exist for special cases: `api_endpoint` overrides the Monarch GraphQL endpoint, and `audit_log` is accepted but not enforced yet (see [key reference](#key-reference)).
...
{"ok":true,"data":{"version":"v0.8.1-...","commit":"b0bdf52","date":"2026-08-21T11:09:16Z","built_by":"unknown"},"meta":{"command":"version","profile":"work","duration_ms":0,"schema_version":"2026-05-08","request_id":"1b320613-4a34-4d8b-9fac-02a2dfd0e91d"}}
```
...
| `read_only` | bool | `false` | parsed, **not enforced** — use `--read-only` or `MONARCH_READONLY=1` |
| `session_path` | string | `<state dir>/session.json` | parsed, **not enforced** |
| `audit_log` | bool | `true` | parsed, **not enforced** — audit logging is always on |
| `output` | string | *(empty)* | parsed, **not enforced** — use `--json`/`--pretty` |

The "not enforced" rows are keys the loader accepts so an old config file still parses, but current code does not act on them. Do not rely on them; use the flag/env equivalents listed above.
...
| `MONARCH_SESSION_PATH` | `session_path` (parsed, not enforced) |
...
{"ok":false,"error":{"code":"READ_ONLY_VIOLATION","message":"remote writes are blocked in read-only mode","category":"safety","retryable":false},"meta":{"command":"transactions.delete","profile":"default","duration_ms":0,"schema_version":"2026-05-08","request_id":"d9b7d82f-10aa-4b5e-bed7-ad48d1fcc98b"}}
$ echo $?
4
```
